...
async def try_book_winbox_slot(page, slot_element, slot_text: str, date: str, config: SiteConfig) -> bool:
            ...
            for indicator in success_indicators:
                try:
                    ...
                    if success_element and await success_element.is_visible():
                        ...
                        account = WINBOX_ACCOUNTS[config.account_index]
                        booking_message = (
                            f"✅ УСПЕШНОЕ БРОНИРОВАНИЕ!\n\n"
                            f"🏢 {config.name}\n"
                            f"📅 Дата: {date}\n"
                            f"⏰ Время: {slot_text}\n"
                            f"👤 Аккаунт: {account['name']}"
                        )
                        await send_notification(booking_message)
...
async def book_winbox_slots(page, date: str, config: SiteConfig) -> List[str]:
    ...
    try:
        ...
        for slot_element, slot_text, hour, minute in slots_with_time:
            ...
            try:
                if await try_book_winbox_slot(page, slot_element, slot_text, date, config):
...
async def determine_slot_availability(text: str, container) -> str:
    ...
    # ✅ ТОЧНАЯ ПРОВЕРКА "Осталось X мест" (высший приоритет)
    remains_patterns = [
        r'осталось\s+(\d+)\s+мест[оа]?',  # "Осталось 1 место"
        r'осталось\s+(\d+)',               # "Осталось 1"
        r'остается\s+(\d+)\s+мест[оа]?',   # "Остаётся 2 места"
    ]
    ...
    # ✅ Проверка "нет мест" (высший приоритет)
    if 'нет мест' in text_lower:
        print(f"          🎯 НАЙДЕН КЛЮЧЕВОЙ ИНДИКАТОР: 'Нет мест'")
        return "unavailable"
    
    # ✅ ЯВНЫЕ ИНДИКАТОРЫ НЕДОСТУПНОСТИ
